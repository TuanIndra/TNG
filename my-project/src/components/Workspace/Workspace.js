...
const Workspace = ({ onDropBlock, blocks, onBlockDrag, onBlockClick   }) => {
  ...
  const [{ isOver }, drop] = useDrop({
    ...
    drop: (item, monitor) => {
      if (dropRef.current) {
        const clientOffset = monitor.getClientOffset(); // Tọa độ chuột khi thả
        const workspaceOffset = dropRef.current.getBoundingClientRect(); // Tọa độ vùng Workspace
        ...
        onDropBlock({
          ...item,
          position: { x: relativeX, y: relativeY }, // Tọa độ block trong Workspace
        });
      }
    },
    collect: (monitor) => ({
